    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional TERANO outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes TERANO and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::TERANO_TO_TERANO) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_terano);
        }
    } else {
        // First try to construct a TERANO-to-TERANO transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::TERANO_ONLY;
        ...
        // If TERANO-to-TERANO fails, create a peg-out transaction
