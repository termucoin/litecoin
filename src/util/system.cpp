...
const char * const BITCOIN_CONF_FILENAME = "teranocoin.conf";
...
#else
    const char* pszModule = "teranocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Teranocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Teranocoin";
#else
    // Unix-like
    return pathRet / ".teranocoin";
#endif
#endif
}
