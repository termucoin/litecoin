...
The Teranocoin Core daemon can be configured to act as a trusted "border
router", implementing the teranocoin wire protocol and relay, making
...
The ZeroMQ feature in Teranocoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building teranocoind:
...
For instance:
    ...
    $ teranocoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/teranocoind.tx.raw \
...
These options can also be provided in teranocoin.conf.
...
From the perspective of teranocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into teranocoind directly. Furthermore, no information is
...
using. Teranocoind appends an up-counting sequence number to each
